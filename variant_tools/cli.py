...
from .vcf_tools import region_to_sequence, generate_accession_dict
...
def vcf2sequence(vcf, sample=None, region=None, region_file=None):
    ...
    if region:
        print(region_to_sequence(reader, region, sample_idx, acdict))

    if region_file:
        for reg in regions_from_file(region_file):
            print(region_to_sequence(reader, reg, sample_idx, acdict))
